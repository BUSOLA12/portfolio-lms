...
- **Open: the invitation lifetime is an engineering default.** Seven days — long enough to survive a weekend and an unchecked inbox, short enough that a forwarded email stops working. No document specifies one. Overridable with `GUARDIAN_INVITATION_TTL_DAYS`, and worth confirming before 1.6 sends real invitations.
