...
### 1.12 Email verification and CORS — **DONE**
...
- **Files:** `apps/api/src/services/verificationService.js`, `apps/api/src/middleware/cors.js`, `packages/schemas/verification.js`, and `apps/api/test/verification.test.js`, which the list omitted but which holds both halves of the done-when. Wiring landed where it was expected: `apps/api/src/routes/auth.js` (the route, behind `claimLimiter`), `apps/api/src/controllers/authController.js` (`verifyEmail`) and `apps/api/src/app.js` (the middleware, mounted ahead of the body parser). Also `packages/schemas/index.js`, re-exporting the schema the way every other one is reached. Nothing else changed: `EMAIL_VERIFICATION` and its TTL were already on `invitationService.js` from 1.6, `WEB_ORIGIN` was already in `.env.example`, and `claimLimiter` already existed. No migration.
...
- **Verified:** `npm run lint` (ESLint then stylelint) exits 0 and `npm run format:check` reports all matched files clean. This step's eleven tests all pass — six over the verification link (stamps once, burns the token rather than deleting it, replay fails without moving the timestamp, an expired link is refused, an unknown and a missing token are refused separately, an account that is not `enabled` is refused) and five over cross-origin access (the web origin may call with credentials and read the reply, a JSON POST preflight is answered, a foreign origin is refused, a request with no `Origin` header is left alone, and an unset `WEB_ORIGIN` refuses every origin per D20).
- **The full suite stands at 113 of 115, and the two failures are the remote database rather than the code.** Both are Prisma's default interactive transaction timeout of 5000 ms being exceeded on a link where a round trip can take seconds — `12077 ms passed since the start of the transaction` in one, `8175 ms` in the other — after which the request returns 500 and the assertion reads `500 !== 422`. One falls in 1.7's claim suite, which this step does not touch; the other is this step's own replay test, which passed on an earlier run of the same code. Every test in this step has therefore been seen to pass, but not all in one run.
- **Two environment faults were found and fixed while verifying, and a third was not.** The first: `apps/api/package.json` now passes `--test-concurrency=1`, because Node's runner defaults to one process per file and each opens its own Prisma pool — seventeen connections apiece — which Railway's TCP proxy will not carry, and the resulting `Server has closed the connection` and `Unable to start a transaction` errors have been read as flakiness for several steps. The second: killed test runs left orphaned node processes holding those pools for the best part of an hour, starving later runs of the connections they needed. The third is left alone deliberately: the 5000 ms transaction timeout is a Prisma default, and raising it — or moving the suite to a local Postgres — is a decision about how this project tests, not part of this step.
- **Open: `authSession.test.js` is not idempotent.** Its expired-session test writes a hardcoded token, `expired-token-under-test`, against a unique constraint and never cleans it up, so any interrupted run leaves a row that makes the next run fail on a unique-constraint violation that looks nothing like the real cause. Two such rows were cleared by hand while verifying this step. The test belongs to 1.2 and is not this step's to change.
- **Decided during the step — the CORS layer is written, not installed.** The same line 1.9 took with the rate limiter: this is a header allowlist and a preflight reply, and the `cors` package would be a dependency carrying far more configuration surface than the two behaviours actually wanted.
- **Decided during the step — `WEB_ORIGIN` is a comma-separated list.** One deployment can name a second origin — a preview domain — without loosening anything, because each is still named in full and matched exactly. Trailing slashes are trimmed on both sides so a variable set with one still matches.
- **Decided during the step — a refused preflight is answered 403; a refused simple request is not blocked.** The two are different: a preflight is a question this middleware exists to answer, so answering it plainly beats letting it fall through to the router and report 404 for a route that exists. A simple request cannot be recalled — the server cannot know the browser will discard the response — and refusing outright would also refuse non-browser callers that send an `Origin` header by accident. Omitting the headers is what stops script reading it. `Vary: Origin` is set on every answer, refusals included, so no cache can serve one origin's response to another.
- **Decided during the step — the rate-limit headers are exposed.** `Retry-After` and the three `RateLimit-*` headers are named in `Access-Control-Expose-Headers`. 1.9 duplicated `retryAfter` into the response body precisely because these were unreadable cross-origin; this is what makes the headers themselves usable, and the duplication can stay as the simpler path for the client.
- **Decided during the step — verification does not sign anyone in.** The endpoint issues no auth session. Proving control of an address is not authentication, and whoever opens the link is not necessarily at the device that registered — an email opened on a shared phone would otherwise hand out a session.
- **Decided during the step — an already-verified account burns the token without re-stamping.** Per D5 `email_verified_at` is a timestamp, not a status: it records the first moment control of the address was proven, and moving it would rewrite a fact that has already happened. The token is still consumed, so a link cannot be kept alive by an account that is already verified, and the response is the same either way.
- **Decided during the step — only an `enabled` account may verify.** A suspended account should not be able to complete anything, and a `pending` stub has an invitation to claim rather than an address to confirm — D18 stamps verification as part of that claim. Both fall into the same indistinguishable failure as an expired token.
- **Decided during the step — the token travels in a POST body, not the query string.** A `GET /auth/verify?token=…` would put a live credential in the API's access log and in any proxy between the two. The landing page 1.11 builds reads the token from its own URL and posts it, which keeps it out of the server logs on this side.
- **Open: `WEB_ORIGIN` must be set in Railway.** The same shape as 1.9's `TRUST_PROXY` note and with the opposite failure: unset, the boundary defaults closed and every screen 1.11 builds is refused. It is in `.env.example`; Railway variables are set separately, and `production` still holds none.
