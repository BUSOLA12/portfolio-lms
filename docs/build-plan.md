...
- **Open: no document sets a password policy.** The schema uses an engineering default along NIST 800-63B lines — an 8-character floor, a 128-character ceiling so an unbounded string never reaches the key derivation, and no composition rules. Marked in the file as needing confirmation before launch.
- **Open: `guardianships.relationship` is `NOT NULL` from 1.1, but nothing collects it.** Brief §6.1 names only the guardian's name, email and contact details as the conditional fields, and 1.1 left the column free text because no document specifies the permitted values. That is consistent for this step, which writes no guardianship row — but 1.4 cannot insert one without either an extra form field or a default. Needs an answer before 1.4.
