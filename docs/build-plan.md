...
- **The rest of the suite could not be confirmed in this environment.** The full run reports 115 tests, 93 passing and 22 failing, and every one of the 22 is the remote Postgres rather than the code: thirty-seven connection errors across the log — `Server has closed the connection`, `Unable to start a transaction in the given time`, `Can't reach database server` — and the eight assertion failures are all a `500` where a `200` was expected, which is what a request downstream of an unreachable database returns. The failures fall in 1.4, 1.7 and 1.8's suites, which this step does not touch, and the same suites stood at 104 passing at 1.10. Worth one clean run when the database link is healthy before this is trusted.
