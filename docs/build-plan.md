...
- **Note on what is being limited.** Two different risks share this router. Login and the claim are guessing attacks, where the limit protects an account and should key on the credential being tried. Password reset and the invitation resend send email to an address the caller supplies, where the limit protects the sending reputation and the recipient, and should key on the address as well as the source. A single limiter tuned for login will be wrong for one of them.
